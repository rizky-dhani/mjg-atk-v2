...
class ApprovalMarketingMediaStockRequest extends ListRecords
{
    protected static string $resource = MarketingMediaStockRequestResource::class;

    protected static ?string $slug = 'marketing-media/stock-requests/approval';

    protected static ?string $navigationLabel = 'Approval Permintaan Marketing Media';

    protected static string|UnitEnum|null $navigationGroup = 'Marketing Media';

    protected static string|BackedEnum|null $navigationIcon = Heroicon::DocumentCheck;

    protected static ?string $title = 'Approval Permintaan Marketing Media';
    ...
    public static function getNavigationBadge(): ?string
    {
        ...
        $count = Approval::whereIn('current_step', $matchingStepIds)
            ...
            ->where('approvable_type', (new MarketingMediaStockRequest)->getMorphClass()) // Ensure it's for Marketing Media Stock Requests
            ->count();
        ...
        return $table
            ...
            ->actions([
                ApprovalAction::makeApprove()->successNotification(
                    Notification::make()
                        ->title('Permintaan Marketing Media berhasil disetujui!')
                        ->success(),
                ),
                ApprovalAction::makeReject()->successNotification(
                    Notification::make()
                        ->title('Permintaan Marketing Media berhasil ditolak!')
                        ->success(),
                ),
            ]);
    }
    ...
    protected function getQuery(): Builder
    {
        ...
        $user = Auth::user();
        if (! $user) {
            return MarketingMediaStockRequest::query()->whereRaw('0=1'); // Return empty query if no user
        }

        // Get all pending MarketingMediaStockRequest records
        ...
        foreach (MarketingMediaStockRequest::whereHas('approval', function ($q) {
            $q->where('status', 'pending');
        })->get() as $stockRequest) {
            if ($approvalService->canUserApproveMarketingMediaStockRequest($stockRequest, $user)) {
